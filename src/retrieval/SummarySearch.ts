...
export class SummarySearch {
  ...
  search(agentId: string, query: string, limit = 10): SummarySearchHit[] {
    return this.summaries
      ...
      .map((summary, originalIndex) => {
        ...
        const authoritativeMaterialRawCount = rawCandidates.filter((raw) => this.isAuthoritativeMaterial(raw)).length;
  ...
  private isAuthoritativeMaterial(raw: RawMessage): boolean {
    return (
      raw.retrievalAllowed &&
      raw.sourcePurpose === "material_corpus" &&
      raw.sourceAuthority === "original_user_supplied_material"
    );
  }
}
