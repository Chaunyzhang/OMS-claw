...
describe("realtest batch summary", () => {
  it("aggregates verdicts, query traces, and sender timings into a compact summary", () => {
    ...
    try {
      ...
      expect(summary.pipeline.totalRuns).toBe(2);
      expect(summary.pipeline.envFailedRuns).toBe(0);
      expect(summary.pipeline.runnableRuns).toBe(2);
      expect(summary.pipeline.validRuns).toBe(2);
      expect(summary.pipeline.invalidRuns).toBe(0);
      expect(summary.pipeline.passRateOnAllRuns).toBe(50);
      expect(summary.pipeline.passRateOnRunnableRuns).toBe(50);
      expect(summary.capability.denominator).toBe("valid_runs_only");
      expect(summary.capability.qualifiedRuns).toBe(2);
      expect(summary.capability.overallPassRate).toBe(50);
      expect(summary.capability.answerCorrectRate).toBe(50);
      expect(summary.capability.authoritativeEvidenceRate).toBe(50);
